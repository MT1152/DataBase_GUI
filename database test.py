...
# Generate ERD using NetworkX
# Generate ERD using NetworkX with node types
def generate_erd(schema, foreign_keys):
    G = nx.DiGraph()  # Directed graph for better representation of relationships
...
style = ttk.Style()
style.configure("Output.TLabel", font=("Arial", 14), foreground="black")  # Configure the font color

output_frame = ttk.Frame(root)
output_frame.pack(pady=20, padx=20, fill=tk.X)

# Apply the custom style to the label inside output_frame
ttk.Label(output_frame, text="Output:", style="Output.TLabel").pack(pady=5)

# Scrollable text widget for displaying query results
